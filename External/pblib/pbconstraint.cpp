...
void PBConstraint::print(bool use_stderr) const
{
  ...
  for (int i = 0; i < getN(); ++i)
  {
	...
	if (use_stderr) cerr << weighted_literals[i].weight << " -x" << -weighted_literals[i].lit << " +";
	...
	if (use_stderr) cerr << weighted_literals[getN() - 1].weight << " -x" << -weighted_literals[getN() - 1].lit;
  ...
  if (comparator == LEQ)
    if(use_stderr) cerr << " =< " << leq << endl;
    else cout << " =< " << leq << endl;
  else if (comparator == GEQ)
    if(use_stderr) cerr << " >= " << geq << endl;
    else cout << " >= " << geq << endl;
  else
    if(use_stderr) cerr << " >= " << geq << " =< " << leq << endl;
    else cout << " >= " << geq << " =< " << leq << endl;
  
}

void PBConstraint::printGeq(bool use_stderr) const
{
  if (comparator != LEQ)
  {
    print();
...
void PBConstraint::printNoNL(bool use_stderr) const
{
  if (getN() == 0)
  {
    if (use_stderr)
  ...
  if (comparator == LEQ)
    if(use_stderr) cerr << " =< " << leq << " ";
    else cout << " =< " << leq << " ";
  else if (comparator == GEQ)
    if(use_stderr) cerr << " >= " << geq << " ";
    else cout << " >= " << geq << " ";
  else
    if(use_stderr) cerr << " >= " << geq << " =< " << leq << " ";
    else cout << " >= " << geq << " =< " << leq << " ";
